...
const protectedPaths = ['/dashboard', '/analytics', '/call-history', '/contacts', '/settings'];
...
function AlertRow({ alert }) {
  return <a className="alert-row" href={`/calls/${alert.id}`}><span className={`alert-indicator ${alert.score >= 75 ? 'high' : ''}`} /><span className="alert-main"><strong>{alert.caller}</strong><small>{alert.time}</small></span><span className="alert-level">{alert.level}</span><span className="alert-score">{alert.score}<small>/100</small></span><span className="alert-arrow" aria-hidden="true">↗</span></a>;
}
...
function TimeHeatmap({ pattern }) {
  const max = Math.max(...pattern.values.flat());
  return <div className="heatmap"><div className="heatmap-grid"><span /><div className="heatmap-days">{pattern.days.map((day) => <span key={day}>{day}</span>)}</div>{pattern.periods.map((period, rowIndex) => <div className="heatmap-row" key={period}><span>{period}</span>{pattern.values[rowIndex].map((value, columnIndex) => <i key={`${period}-${pattern.days[columnIndex]}`} className={`heat-${Math.ceil((value / max) * 4)}`} title={`${value} suspicious calls on ${pattern.days[columnIndex]} ${period.toLowerCase()}`} />)}</div>)}</div><p className="heatmap-note">Darker cells indicate more suspicious calls.</p></div>;
}
...
function CallHistory() {
  ...
  return <main className="dashboard-shell">
    ...
    <div className="dashboard-content history-content">
      ...
      <section className="history-table" aria-label="Past calls"><div className="history-header" aria-hidden="true"><span>Caller</span><span>Date & time</span><span>Duration</span><span>Risk level</span><span>Score</span><span>Detection</span><span /></div>{visibleCalls.length ? visibleCalls.map((call) => <CallHistoryRow key={call.id} call={call} />) : <div className="history-empty"><h2>No calls match these filters</h2><p>Try changing the search or filter range.</p></div>}</section>
...
function CallDetails({ id }) {
  ...
  useEffect(() => {
    api.getCallById(id).then((response) => setReport(response.payload));
  }, [id]);
...
function ContactCard({ contact, onRemove }) {
  return <article className="contact-card"><div className="contact-avatar" aria-hidden="true">{contact.name.split(' ').map((part) => part[0]).join('').slice(0, 2)}</div><div className="contact-info"><h2>{contact.name}</h2><p>{contact.number}</p><span className="verified-label"><i />{contact.verifiedDate}</span></div><button className="remove-contact" type="button" onClick={() => onRemove(contact)}>Remove</button></article>;
}
...
function Contacts() {
  ...
  return <main className="dashboard-shell">
    ...
    <div className="dashboard-content contacts-content">
      ...
      {showForm && <form className="contact-form" onSubmit={addContact}><div><label htmlFor="contact-name">Name</label><input id="contact-name" name="name" value={form.name} onChange={updateForm} placeholder="e.g. Jordan Lee" /></div><div><label htmlFor="contact-number">Phone number</label><input id="contact-number" name="number" type="tel" value={form.number} onChange={updateForm} placeholder="e.g. +1 (202) 555-0161" /></div><button type="submit">Verify and add</button>{formError && <p role="alert">{formError}</p>}</form>}
...
function App() {
  const isAuthenticated = () => window.localStorage.getItem('voiceguard-authenticated') === 'true';
  ...
  useEffect(() => {
    ...
    const handleNavigation = (event) => {
      ...
      event.preventDefault();
      if (link.dataset.signout === 'true') window.localStorage.removeItem('voiceguard-authenticated');
  ...
  if (currentPath === '/') page = <><Navbar /><Hero /></>;
    ...
    const detailMatch = currentPath.match(/^\/call-history\/([^/]+)$/);
    ...
    else page = <AuthPage register={currentPath === '/register'} />;
  }
